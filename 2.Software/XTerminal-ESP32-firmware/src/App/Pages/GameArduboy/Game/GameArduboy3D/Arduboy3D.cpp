...
Arduboy2Base arduboy_game2;
ArduboyTones sound(arduboy_game2.audio.enabled);
Sprites sprites;
...
uint8_t Platform::GetInput()
{
  ...
  if(arduboy_game2.pressed(A_BUTTON))
  {
    result |= INPUT_A;  
  }
  if(arduboy_game2.pressed(B_BUTTON))
  {
    result |= INPUT_B;  
  }
  if(arduboy_game2.pressed(UP_BUTTON))
  {
    result |= INPUT_UP;  
  }
  if(arduboy_game2.pressed(DOWN_BUTTON))
  {
    result |= INPUT_DOWN;  
  }
  if(arduboy_game2.pressed(LEFT_BUTTON))
  {
    result |= INPUT_LEFT;  
  }
  if(arduboy_game2.pressed(RIGHT_BUTTON))
  {
    result |= INPUT_RIGHT;  
  }
...
{
  arduboy_game2.digitalWriteRGB(r ? RGB_ON : RGB_OFF, g ? RGB_ON : RGB_OFF, b ? RGB_ON : RGB_OFF);
}

void Platform::PutPixel(uint8_t x, uint8_t y, uint8_t colour)
{
  arduboy_game2.drawPixel(x, y, colour);
}
...
{
  uint8_t *screenptr = arduboy_game2.getBuffer() + x;
...
{
  return arduboy_game2.getBuffer();
}
...
void Platform::DrawBitmap(int16_t x, int16_t y, const uint8_t *bitmap)
{
  ...
  arduboy_game2.drawBitmap(x, y, bitmap + 2, w, h);
}
...
void Platform::DrawSolidBitmap(int16_t x, int16_t y, const uint8_t *bitmap)
{
  uint8_t w = pgm_read_byte(&bitmap[0]);
  uint8_t h = pgm_read_byte(&bitmap[1]);
  arduboy_game2.fillRect(x, y, w, h, BLACK);
  arduboy_game2.drawBitmap(x, y, bitmap + 2, w, h);
}

void Platform::FillScreen(uint8_t colour)
{
  arduboy_game2.fillScreen(colour);
}
...
{
	return arduboy_game2.audio.enabled();
}

void Platform::SetAudioEnabled(bool isEnabled)
{
	if(isEnabled)
		arduboy_game2.audio.on();
	else
		arduboy_game2.audio.off();
}
...
void GameArduboy3D_setup()
{
  arduboy_game2.boot();
  arduboy_game2.flashlight();
  arduboy_game2.systemButtons();
  //arduboy.bootLogo();
  arduboy_game2.setFrameRate(TARGET_FRAMERATE);
